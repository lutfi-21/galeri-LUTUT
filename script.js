...
function checkPass() {
    if (document.getElementById('passInput').value === 'LutfiDita2026') {
        ...
        // TAMPILKAN VIDEO KEJUTAN DULU
        const vOverlay = document.getElementById('videoOverlay');
        const vSurprise = document.getElementById('surpriseVideo');
...
function loadPhotos() {
    database.ref('photos').on('child_added', (snapshot) => {
        ...
        div.innerHTML = `<button class="delete-btn" onclick="deletePhoto('${snapshot.key}')">&times;</button><img src="${data.image}"><div class="photo-caption">${data.caption || ''}</div>`;
...
function startLoveCounter() {
    ...
    setInterval(() => {
        ...
        document.getElementById('loveCounter').innerHTML = `Sudah <span>${d}D</span> <span>${h}H</span> <span>${m}M</span> <span>${s}S</span> ❤️`;
    }, 1000);
}
...
function handleUpload(event) {
    ...
    if (file) {
        ...
        reader.onload = (e) => { database.ref('photos').push({ image: e.target.result, caption: document.getElementById('photoCaption').value || "❤️" }); };
